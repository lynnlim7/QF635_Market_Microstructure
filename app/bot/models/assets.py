...
class Assets(Base) : 
    ...
    spot_pairs_as_base = relationship(
        "SpotPairs",
        foreign_keys="SpotPairs.base_asset_id",
        back_populates="base_asset"
    )

    spot_pairs_as_quote = relationship(
        "SpotPairs",
        foreign_keys="SpotPairs.quote_asset_id",
        back_populates="quote_asset"
    )

    futures_pairs_as_base = relationship(
        "FuturesPairs",
        foreign_keys="FuturesPairs.base_asset_id",
        back_populates="base_asset"
    )

    futures_pairs_as_quote = relationship(
        "FuturesPairs",
        foreign_keys="FuturesPairs.quote_asset_id",
        back_populates="quote_asset"
    )
...
class SpotPairs(Base, PairsMixin) : 
    ...
    base_asset = relationship(
        "Assets",
        foreign_keys=[PairsMixin.base_asset_id],
        back_populates="spot_pairs_as_base"
    )

    quote_asset = relationship(
        "Assets",
        foreign_keys=[PairsMixin.quote_asset_id],
        back_populates="spot_pairs_as_quote"
    )
